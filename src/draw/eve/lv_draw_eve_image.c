...
void lv_draw_eve_image(lv_draw_eve_unit_t * draw_unit, const lv_draw_image_dsc_t * draw_dsc, const lv_area_t * coords)
{
    ...
    if(draw_dsc->rotation || draw_dsc->scale_x != LV_SCALE_NONE || draw_dsc->scale_y != LV_SCALE_NONE) {
        ...
        EVE_cmd_translate_burst(F16(coords->x1 - draw_unit->base_unit.clip_area->x1 + draw_dsc->pivot.x),
                                F16(coords->y1 - draw_unit->base_unit.clip_area->y1 + draw_dsc->pivot.y));
        ...
        EVE_cmd_dl_burst(CMD_LOADIDENTITY);
        eve_vertex_2f(draw_unit->base_unit.clip_area->x1, draw_unit->base_unit.clip_area->y1);
    }
    else {
        eve_vertex_2f(coords->x1, coords->y1);
    }
    eve_restore_context();
    EVE_end_cmd_burst();
    EVE_execute_cmd();
